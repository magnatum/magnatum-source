...
public:
    // header
    static const unsigned short CURRENT_VERSION = 0x12;
    unsigned short nVersion;
    unsigned short nRevardDiffSplit;
    ...
    bool ReadFromDisk(unsigned int nFile, unsigned int nBlockPos, bool fReadTransactions=true)
    {
        ...
        // Check the header
        if (fReadTransactions && IsProofOfWork() && !CheckProofOfWork(GetPoWHash(), nBits, nRevardDiffSplit))
    ...
    {
        std::stringstream s;
        s << strprintf("CBlock(hash=%s, ver=%d, hashPrevBlock=%s, hashMerkleRoot=%s, nTime=%u, nBits=%08x, nNonce=%u, split=%u, vtx=%u, vchBlockSig=%s)\n",
            GetHash().ToString(),
            nVersion,
            hashPrevBlock.ToString(),
            hashMerkleRoot.ToString(),
            nTime, nBits, nNonce, nRevardDiffSplit,
            vtx.size(),
...
public:
    ...
    int nHeight;
    unsigned int nRevardDiffSplit; // bolondz: split reward and dificulty when net is overhead
    unsigned int nSize; // bolondz: used to calculate split
    ...
    CBlockIndex(unsigned int nFileIn, unsigned int nBlockPosIn, CBlock& block)
    {
        ...
        nBits          = block.nBits;
        nNonce         = block.nNonce;
        nRevardDiffSplit = block.nRevardDiffSplit;
        nSize = block.GetSerializeSize(SER_DISK, CLIENT_VERSION);
    }
    ...
    CBlock GetBlockHeader() const
    {
        ...
        block.nTime          = nTime;
        block.nBits          = nBits;
        block.nRevardDiffSplit = nRevardDiffSplit;
        block.nNonce         = nNonce;
        return block;
    }
    ...
    unsigned int GetNextSplit() const
    {
        if(nRevardDiffSplit > 1 && nSize < MAX_BLOCK_SIZE / 2)
        {
            return nRevardDiffSplit / 2;
        }
        else if(nSize > MAX_BLOCK_SIZE / 2)
        {
            return nRevardDiffSplit * 2;
        }
        return nRevardDiffSplit;
    }
};
