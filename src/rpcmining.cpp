...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfWorkReward(0, pindexBest ? pindexBest->GetNextSplit() : 1);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("blockvalue",    (uint64_t)GetProofOfWorkReward(0, pindexBest ? pindexBest->GetNextSplit() : 1)));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Magnatum is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Magnatum is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Magnatum is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Magnatum is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    Object result;
    int nVersion = (pblock->nVersion) | (pblock->nRevardDiffSplit << 16);
