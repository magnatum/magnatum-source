...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("magnatum"))
        return false;
...
{
    // Convert magnatum:// to magnatum:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("magnatum://"))
    {
        uri.replace(0, 12, "magnatum:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Magnatum.lnk";
}
...
{
    return GetAutostartDir() / "magnatum.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Magnatum-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  magnatum-qt [" + tr("command-line options") + "]                     " + "\n";
