...
#if defined(NDEBUG)
# error "Magnatum cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Magnatum Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees, unsigned int nRevardDiffSplit)
{
    int64_t coin = COIN / nRevardDiffSplit;
   ...
   // BlockCreating = BlockCreating;
    ...
    for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
        ...
        if(LastBlockTime > 0){
            ...
            nActualTimespan += Diff*BlockReading->nRevardDiffSplit;
        }
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, unsigned int nRevardDiffSplit)
{
    CBigNum bnTarget;
    bnTarget.SetCompact(nBits);
    bnTarget /= nRevardDiffSplit;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfWork())
    {
        if(nRevardDiffSplit != pindex->GetNextSplit())
            return error("ConnectBlock() : Invalid block split (actual = %u, calculated = %u", nRevardDiffSplit, pindex->GetNextSplit());

        int64_t nReward = GetProofOfWorkReward(nFees, nRevardDiffSplit);
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    // Check proof of work matches claimed amount
    if (fCheckPOW && IsProofOfWork() && !CheckProofOfWork(GetPoWHash(), nBits, nRevardDiffSplit))
...
bool CBlock::AcceptBlock()
{
    ...
    if (GetBlockTime() > FutureDrift((int64_t)vtx[0].nTime, nHeight))
        return DoS(50, error("AcceptBlock() : coinbase timestamp is too early %l, %l", GetBlockTime(), FutureDrift((int64_t)vtx[0].nTime, nHeight)));
    ...
    // Check split
    if (nRevardDiffSplit != pindexPrev->GetNextSplit())
        return error("AcceptBlock() : invalid split %u vs %u", nRevardDiffSplit, pindexPrev->GetNextSplit());
    ...
    if (vtx[0].vin[0].scriptSig.size() < expect.size() ||
        ...
        return DoS(100, error("AcceptBlock() : block height mismatch in coinbase %s %s", HexStr(expect), HexStr(vtx[0].vin[0].scriptSig)));
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (pblock->hashPrevBlock != hashBestChain)
    {
        ...
        if (deltaTime < 0)
        {
            if (pfrom)
                pfrom->Misbehaving(1);
            return error("ProcessBlock() : block with timestamp before last checkpoint %ld %ld", pblock->GetBlockTime(), pcheckpoint->nTime);
        }
    }
